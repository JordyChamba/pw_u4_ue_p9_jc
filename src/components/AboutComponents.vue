<template>
    <div class="container">
        ...
        <div class="section">
            ...
            <input v-model="estudiante.fechaNacimiento" type="datetime-local" placeholder="Fecha Nacimiento" />
        ...
        <div class="section">
            ...
            <button @click="actualizarParcial">Actualizar Parcial</button> <!-- nuevo botón -->
        ...
        <table border="1" cellspacing="0" cellpadding="5">
            ...
            <tbody>
                <tr v-for="e in estudiantes" :key="e.id" @click="cargarEstudianteEnInputs(e)" style="cursor:pointer">
                    <td>{{ e.id }}</td>
                    <td>{{ e.nombre }}</td>
                    <td>{{ e.apellido }}</td>
...
export default {
    ...
    methods: {
        ...
        async consultarHijos(estudianteId) {
            try {
                const response = await fetch(`http://localhost:8081/matricula/api/v1.0/estudiantes/${estudianteId}/hijos`);
                if (!response.ok) {
                    return [];
                }
        ...
        async guardar() {
            ...
            if (fecha.length === 16) { // yyyy-MM-ddTHH:mm
        ...
        async actualizarParcial() {
            if (!this.id) {
                alert('Por favor selecciona un estudiante para actualizar parcialmente');
                return;
            }
            try {
                let fecha = this.estudiante.fechaNacimiento;
