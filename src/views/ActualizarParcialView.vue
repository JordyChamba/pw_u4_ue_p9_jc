    ...
    <div class="container">
        ...
        <div v-if="estudiante.id" class="form-section">
            ...
            <div class="button-group">
                <button @click="actualizarParcial" class="btn-success">Actualizar Parcialmente</button>
                <button @click="limpiarFormulario" class="btn-secondary">Cancelar</button>
            </div>
        </div>
        <p v-else class="message">Busque un estudiante para actualizar parcialmente</p>
    </div>
</template>

<script>
import { consutarPorIDFachada } from '@/clients/matricula.js';
...
export default {
    ...
    methods: {
        async buscarEstudiante() {
            ...
            try {
                const datos = await consutarPorIDFachada(this.id);
        ...
        async actualizarParcial() {
            ...
            try {
                ...
                // Llamar a endpoint PATCH (debes tener actualizarParcialFachada en tu cliente)
                const response = await fetch(`http://localhost:8081/matricula/api/v1.0/estudiantes/${this.id}`, {
                    method: 'PATCH',
                    headers: {
                ...
                alert('Estudiante actualizado parcialmente con éxito');
                this.limpiarFormulario();
            } catch (error) {
                console.error(error);
                alert('Error al actualizar estudiante parcialmente');
            }
        },
